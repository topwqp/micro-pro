...
import com.xs.micro.invoked.extension.exception.BadOperationException;
import com.xs.micro.invoked.extension.exception.ProcessingException;
...
public class GlobalExceptionHandler {
    ...
    public GlobalResponseEntity jsonErrorHandler(HttpServletRequest request, RuntimeException e) {
        String requestURI = request.getRequestURI();
        Map<String, String[]> parameterMap = request.getParameterMap();
        LOG.error(String.format("request Uri :[%s] handle exception runtime xxxxxxxxx... params:[%s]",requestURI,JSON.toJSONString(parameterMap)),e);
    ...
    public GlobalResponseEntity exceptionHandler(HttpServletRequest request, Exception e) {
        String requestURI = request.getRequestURI();
        Map<String, String[]> parameterMap = request.getParameterMap();
        LOG.error(String.format("request Uri :[%s]  occur exception ... params:[%s]",requestURI,JSON.toJSONString(parameterMap)),e);
        return GlobalResponseEntity.failure(e.getMessage());
    }
}
